...
export class WorkflowService {
    ...
    private async runWorkflowAsync(resourceManager: ResourceManager,
        ...
        // set metadata type (DescriptiveMetadata is missing the "@type" property for some reason)
        metadata["@type"] = "DescriptiveMetadata";

        const inputFile = new Locator({
            awsS3Bucket: uploadBucket,
            awsS3Key: objectKey
        });
        // use type of "S3Locator" - .NET version of MCMA specifies Locator types, and this should
        // probably be ported to the js version as well
        inputFile["@type"] = "S3Locator";

        // creating workflow job
        let workflowJob = new WorkflowJob(
            jobProfileId,
            new JobParameterBag({ metadata, inputFile })
        );
