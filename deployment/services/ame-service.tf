...
resource "aws_lambda_function" "ame-service-api-handler" {
  filename         = "./../services/Mcma.Aws.AmeService/ApiHandler/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-ame-service-api-handler")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.AmeService.ApiHandler::Mcma.Aws.AmeService.ApiHandler.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.AmeService/ApiHandler/dist/lambda.zip"))}"
...
resource "aws_lambda_function" "ame-service-worker" {
  filename         = "./../services/Mcma.Aws.AmeService/Worker/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-ame-service-worker")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.AmeService.Worker::Mcma.Aws.AmeService.Worker.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.AmeService/Worker/dist/lambda.zip"))}"
  runtime          = "dotnetcore2.1"
  timeout          = "300"
  memory_size      = "3008"
}
