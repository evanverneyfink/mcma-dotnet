...
resource "aws_lambda_function" "workflow-service-api-handler" {
  filename         = "./../services/Mcma.Aws.WorkflowService/ApiHandler/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-workflow-service-api-handler")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.WorkflowService.ApiHandler::Mcma.Aws.WorkflowService.ApiHandler.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.WorkflowService/ApiHandler/dist/lambda.zip"))}"
...
resource "aws_lambda_function" "workflow-service-worker" {
  filename         = "./../services/Mcma.Aws.WorkflowService/Worker/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-workflow-service-worker")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.WorkflowService.Worker::Mcma.Aws.WorkflowService.Worker.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.WorkflowService/Worker/dist/lambda.zip"))}"
  runtime          = "dotnetcore2.1"
  timeout          = "300"
  memory_size      = "3008"
}
...
resource "aws_api_gateway_integration" "workflow_service_api_method-integration" {
  rest_api_id             = "${aws_api_gateway_rest_api.workflow_service_api.id}"
  resource_id             = "${aws_api_gateway_resource.workflow_service_api_resource.id}"
...
resource "aws_api_gateway_deployment" "workflow_service_deployment" {
  depends_on = [
    "aws_api_gateway_method.workflow_service_api_method",
    "aws_api_gateway_integration.workflow_service_api_method-integration",
  ]
