...
resource "aws_lambda_function" "job-repository-api-handler" {
  filename         = "./../services/Mcma.Aws.JobRepository/ApiHandler/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-job-repository-api-handler")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.JobRepository.ApiHandler::Mcma.Aws.JobRepository.ApiHandler.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.JobRepository/ApiHandler/dist/lambda.zip"))}"
...
resource "aws_lambda_function" "job-repository-worker" {
  filename         = "./../services/Mcma.Aws.JobRepository/Worker/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-job-repository-worker")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.JobRepository.Worker::Mcma.Aws.JobRepository.Worker.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.JobRepository/Worker/dist/lambda.zip"))}"
  runtime          = "dotnetcore2.1"
  timeout          = "30"
  memory_size      = "256"
}
