...
resource "aws_lambda_function" "job-processor-service-api-handler" {
  filename         = "./../services/Mcma.Aws.JobProcessor/ApiHandler/dist/lambda.zip"
  ...
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.JobProcessor/ApiHandler/dist/lambda.zip"))}"
...
resource "aws_lambda_function" "job-processor-service-worker" {
  filename         = "./../services/Mcma.Aws.JobProcessor/Worker/dist/lambda.zip"
  ...
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.JobProcessor/Worker/dist/lambda.zip"))}"
  runtime          = "dotnetcore2.1"
  timeout          = "30"
  memory_size      = "256"
}
