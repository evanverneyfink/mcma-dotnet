...
resource "aws_lambda_function" "transform-service-api-handler" {
  filename         = "./../services/Mcma.Aws.TransformService/ApiHandler/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-transform-service-api-handler")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.TransformService.ApiHandler::Mcma.Aws.TransformService.ApiHandler.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.TransformService/ApiHandler/dist/lambda.zip"))}"
...
resource "aws_lambda_function" "transform-service-worker" {
  filename         = "./../services/Mcma.Aws.TransformService/Worker/dist/lambda.zip"
  function_name    = "${format("%.64s", "${var.global_prefix}-transform-service-worker")}"
  role             = "${aws_iam_role.iam_for_exec_lambda.arn}"
  handler          = "Mcma.Aws.TransformService.Worker::Mcma.Aws.TransformService.Worker.Function::Handler"
  source_code_hash = "${base64sha256(file("./../services/Mcma.Aws.TransformService/Worker/dist/lambda.zip"))}"
  runtime          = "dotnetcore2.1"
  timeout          = "300"
  memory_size      = "3008"
}
